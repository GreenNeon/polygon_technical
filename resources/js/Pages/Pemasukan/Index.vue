<template>
	<div class="py-3">
		<portal to="header-name"> Dashboard Pemasukan </portal>
		<div class="card-tool">
			<v-row align="center" justify-lg="center" class="px-4 md:px-0 mt-0.5">
				...
				<v-col cols="12" sm="2" md="1">
					<v-btn
						color="primary"
						elevation="2"
						:href="route('dashboard.pemasukan.create')"
						>Buat Baru</v-btn
					>
				</v-col>
			</v-row>
		...
		<div class="card-table">
			...
			>
				<template v-slot:item.actions="{ item }">
					<inertia-link :href="route('dashboard.pemasukan.show', [item.id])">
						<v-icon small class="mr-2"> mdi-eye </v-icon>
					</inertia-link>

					<inertia-link :href="route('dashboard.pemasukan.edit', [item.id])">
...
export default {
	name: "Pemasukan-Admin-Index",
	setup() {},
	props: ["pemasukan_all", "flash"],
	...
	methods: {
		...
		deleteItemConfirm() {
			...
			this.$inertia.delete(
				route("dashboard.pemasukan.destroy", [this.editedItem.id]),
				{
					onSuccess: (page) => {
						this.snackbar.show = true;
						this.snackbar.message = "Berhasil menghapus data pemasukan..";
						this.snackbar.color = "green darken-1 ";
						let id = this.pemasukan_all.findIndex(
							(item) => item.id === this.editedItem.id
						);
						if (id > -1) this.pemasukan_all.splice(id, 1);
						this.editedItem = {};
					},
					onError: (errors) => {
						this.snackbar.show = true;
						this.snackbar.message = "Gagal menghapus data pemasukan..";
